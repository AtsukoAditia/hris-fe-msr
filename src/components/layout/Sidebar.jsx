...
const Sidebar = ({ isOpen, onClose }) => {
      ...
      item.children
        ? {
            ...
            children: item.children.filter((child) =>
              canAccess(child, user?.role),
            ),
          }
        : item,
    );

  const profileGroupActive =
    filteredNavItems
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-30 w-64 bg-white border-r border-gray-200 flex flex-col transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
          ...
          {filteredNavItems.map((item) => {
              ...
              return (
                <div key={item.id}>
                  ...
                  {isExpanded && (
                    <div
                      id={`${item.id}-submenu`}
                      className="mt-1 ml-4 pl-3 border-l border-gray-200 space-y-1"
                    >
                      {item.children.map((child) => (
                        <NavLink
                          ...
                          onClick={onClose}
                          className={({ isActive }) =>
                            `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive ? "bg-indigo-50 text-indigo-700" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`
                          }
                        >
            ...
            return (
              <NavLink
                ...
                onClick={onClose}
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive ? "bg-indigo-50 text-indigo-700 border border-indigo-200" : "text-gray-700 hover:bg-gray-50 hover:text-gray-900"}`
                }
              >
                <span className="text-lg">{item.icon}</span>
